/*------------------------------------------
--------------------------------------------
Middleware
--------------------------------------------
--------------------------------------------*/
require('dotenv').config();
var express = require('express');
var app = express();
const bodyParser = require('body-parser');
const path = require('path');
const fs = require('fs').promises;
const publicPath = path.join(path.resolve(), 'public');
const distPath = path.join(path.resolve(), 'dist');
const isPlatformAdmin = require('./middlewares/platformAdminMiddleware');
const isAuthenticated = require('./middlewares/authMiddleware');

// Database Connection
const conn = require('./config/db');
const xmlbuilder = require('xmlbuilder');
// for querying DB
const util = require('util');
const query = util.promisify(conn.query).bind(conn);

// Initiate Socket IO connection
const http = require('http');
const server = http.createServer(app);

const { createSocket } = require('./config/socketConfig');
createSocket(server);

// Behind the production proxy/load balancer: needed so secure session
// cookies and req.secure work (RFAB_INSTITUTE_BRIDGE.md section 9).
app.set('trust proxy', 1);

// Allow things to work.
var cors = require('cors');
app.use(cors());
// Login with Google - ID tokens are cryptographically VERIFIED (signature +
// audience + expiry) with google-auth-library; jwt.decode() trusted anything.
const { OAuth2Client } = require('google-auth-library');
const GOOGLE_CLIENT_ID =
    process.env.GOOGLE_CLIENT_ID ||
    '13191319610-qectaoi146ce1pm4v95jtgctsbtmqb3t.apps.googleusercontent.com';
const googleAuthClient = new OAuth2Client(GOOGLE_CLIENT_ID);

/**
 * Verify a Google Sign-In credential. Resolves the verified payload; throws
 * on an invalid/forged token or an unverified email (the bridge attaches
 * accounts by email, so email_verified is mandatory).
 */
async function verifyGoogleCredential(credential) {
    const ticket = await googleAuthClient.verifyIdToken({
        idToken: credential,
        audience: GOOGLE_CLIENT_ID
    });
    const payload = ticket.getPayload();
    if (!payload || payload.email_verified !== true) {
        throw new Error('Google account email is not verified');
    }
    return payload;
}

// Limit effects max image size that can be uploaded.
app.use(bodyParser.json({ limit: '100mb' }));
app.use(
    bodyParser.urlencoded({
        limit: '100mb',
        extended: true,
        parameterLimit: 50000
    })
);
app.use(express.json({ limit: '100mb' }));
app.use(express.urlencoded({ limit: '100mb', extended: true }));
app.set('view engine', 'ejs');
app.use(express.static(__dirname + '/public'));
// For Vue Router.
//var history = require('connect-history-api-fallback');

// Sessions: env secret + MariaDB-backed store + hardened cookie flags
// (RFAB_INSTITUTE_BRIDGE.md section 9). The middleware lives in
// config/session.js because Socket.IO shares it (io.engine.use) to
// authenticate socket connections.
const { sessionMiddleware } = require('./config/session');
app.use(sessionMiddleware);

// RFab bridge: shared identity + shared token wallet with rfab.ai.
// Both degrade to fully-local behavior when RFAB_BRIDGE_SECRET is unset.
const rfabBridge = require('./services/rfabBridge');
const identityBridge = require('./services/identityBridge');

// For primary key for users table, for Google signup.
const { v7: uuidv7 } = require('uuid');

/*------------------------------------------
--------------------------------------------
Routes
--------------------------------------------
--------------------------------------------*/
const userRouter = require('./routes/users');
app.use('/users', userRouter);
const skillRouter = require('./routes/skills');
app.use('/skills', skillRouter);
const userSkillRouter = require('./routes/user-skills');
app.use('/user-skills', userSkillRouter);
const resourcesRouter = require('./routes/resources');
app.use('/resources', resourcesRouter);
const userVotesRouter = require('./routes/user-votes');
app.use('/user-votes', userVotesRouter);
const tutorVotesRouter = require('./routes/tutor-votes');
app.use('/tutor-votes', tutorVotesRouter);
const tagRouter = require('./routes/tags');
app.use('/tags', tagRouter);
const skillTagRouter = require('./routes/skill-tags');
app.use('/skill-tags', skillTagRouter);
const questionRouter = require('./routes/questions');
app.use('/questions', questionRouter);
const notificationRouter = require('./routes/notifications');
app.use('/notifications', notificationRouter);
const newsRouter = require('./routes/news');
app.use('/news', newsRouter);
const assessmentsRouter = require('./routes/assessments');
app.use('/assessments', assessmentsRouter);
const unmarkedAnswersRouter = require('./routes/unmarked-answers');
app.use('/unmarked-answers', unmarkedAnswersRouter);
const instructorStudentsRouter = require('./routes/instructor-students');
app.use('/instructor-students', instructorStudentsRouter);
const skillTreeRouter = require('./routes/skilltree');
app.use('/skilltree', skillTreeRouter);
const contentFlagsRouter = require('./routes/content-flags');
app.use('/content-flags', contentFlagsRouter);
const userActions = require('./routes/user-actions');
app.use('/user-actions', userActions);
const tutorPosts = require('./routes/tutor-posts');
app.use('/tutor-posts', tutorPosts);
const skillHistory = require('./routes/skill-history');
app.use('/skill-history', skillHistory);
const todoCount = require('./routes/todo-count');
app.use('/todo-count', todoCount);
const cohorts = require('./routes/cohorts');
app.use('/cohorts', cohorts);
const passwordReset = require('./routes/password-reset');
app.use('/password-reset', passwordReset);
const newSkillsAwaitingApproval = require('./routes/new-skills-awaiting-approval');
app.use('/new-skills-awaiting-approval', newSkillsAwaitingApproval);
const learningTracks = require('./routes/learning-tracks');
app.use('/learning-tracks', learningTracks);
const aiTutor = require('./routes/ai-tutor');
app.use('/ai-tutor', aiTutor);
const skillLearningObjectives = require('./routes/skill-learning-objectives');
app.use('/skill-learning-objectives', skillLearningObjectives);
const tokens = require('./routes/tokens');
app.use('/tokens', tokens);
const referrals = require('./routes/referrals');
app.use('/referrals', referrals);
const studentAnalytics = require('./routes/student-analytics');
app.use('/student-analytics', studentAnalytics);
const tenants = require('./routes/tenants');
app.use('/tenants', tenants);

// error handling middleware
const errorHandlingMiddleware = require('./middlewares/errorHandlingMiddleWare');
app.use(errorHandlingMiddleware);

// To log reasons for crashes
process.on('uncaughtException', (err) => {
    console.error('Uncaught Exception:', err);
});

process.on('unhandledRejection', (reason, promise) => {
    console.error('Unhandled Rejection:', reason);
});

if (process.env.NODE_ENV === 'production') {
    app.use('/', express.static(distPath));
} else {
    app.use('/', express.static(publicPath));
}
const { saveUserAvatarToAWS } = require('./utilities/save-image-to-aws');
/*
 * Login
 */

// Log in / sign up with Google (RFAB_INSTITUTE_BRIDGE.md sections 1-2).
//
// All verification and DB work happens in the POST handlers and the response
// is a redirect the browser follows - to the client this looks exactly like
// the legacy POST -> GET dance, minus the extra hop. The old module-level
// `googleUserDetails` / `googleLoginResult` globals (shared across ALL
// concurrent users - one user could be logged into another user's Google
// account) are gone; transient state lives in the caller's session.

// Default avatar for newly created accounts (MEDIUMTEXT column cannot take a
// DEFAULT value, so it is provided at creation time).
const DEFAULT_AVATAR = `data:image/png;base64,`;

const { unlockInitialSkills } = require('./utilities/unlock-initial-skills');

/** Populate the session after a successful authentication. */
function establishSession(req, user) {
    req.session.isLoggedIn = true;
    req.session.userId = user.id;
    req.session.userName = user.username;
    req.session.firstName = user.first_name;
    req.session.lastName = user.last_name;
    req.session.role = user.role;
}

/**
 * Create an institute account from a verified Google payload (used by both
 * the signup endpoints and the login endpoint's auto-create path).
 * @param {object} payload  verified Google ID-token payload
 * @param {string} role     'student' | 'instructor' | 'editor' (allowlisted by callers)
 * @param {string|null} referrerUsername
 * @return {Promise<object>} the inserted user row data
 */
async function createGoogleUser(payload, role, referrerUsername) {
    const newUserId = uuidv7();
    const data = {
        first_name: payload.given_name,
        last_name: payload.family_name,
        username: payload.email,
        email: payload.email,
        role: role,
        id: newUserId,
        is_google_auth: 1
    };
    await query('INSERT INTO users SET ?', data);

    // Record the referring user, if any ('null'/'undefined' are stringified
    // empties the frontend can send in the query string).
    if (
        referrerUsername &&
        referrerUsername !== 'null' &&
        referrerUsername !== 'undefined'
    ) {
        const referrers = await query(
            'SELECT id FROM users WHERE username = ?',
            [referrerUsername]
        );
        if (referrers.length > 0) {
            await query(
                `INSERT INTO referrals (referred_user_id, referrer_user_id)
                 VALUES (?, ?)
                 ON DUPLICATE KEY UPDATE referred_user_id = VALUES(referred_user_id),
                                         referrer_user_id = VALUES(referrer_user_id)`,
                [newUserId, referrers[0].id]
            );
        }
    }

    // Default avatar + initial skills (same as the legacy signup path).
    await saveUserAvatarToAWS(newUserId, DEFAULT_AVATAR);
    unlockInitialSkills(newUserId).catch((err) =>
        console.error('Error unlocking initial skills:', err)
    );
    return data;
}

// Log in with Google.
app.post('/google-login-attempt', async (req, res, next) => {
    let payload;
    try {
        payload = await verifyGoogleCredential(req.body.credential);
    } catch (err) {
        console.error('Google credential verification failed:', err.message);
        return res.status(401).json({ account: 'invalid-google-token' });
    }
    try {
        const device = req.query.device;
        const users = await query('SELECT * FROM users WHERE email = ?', [
            payload.email
        ]);
        let user = users[0];
        if (user) {
            // Restore soft-deleted accounts and mark the user as
            // authenticating with Google (legacy behavior).
            await query(
                'UPDATE users SET is_deleted = 0, is_google_auth = 1 WHERE email = ?',
                [payload.email]
            );
            establishSession(req, user);
            if (user.is_deleted) req.session.googleLoginResult = 'new account';
        } else {
            // No account yet: a Google login auto-creates a student account
            // (legacy behavior, previously via the signup redirect).
            user = await createGoogleUser(payload, 'student', null);
            establishSession(req, user);
            req.session.googleLoginResult = 'new account';
        }

        // Attach to (or create) the shared RFab identity and settle the
        // one-time balance migration. Best-effort: never blocks the login.
        await identityBridge.ensureGoogleMapping(user.id, payload);

        if (user.role === 'student') {
            return res.redirect(
                device === 'not-mobile' ? '/skill-tree' : '/search'
            );
        }
        return res.redirect('/');
    } catch (err) {
        next(err);
    }
});

app.get('/google-login-result', (req, res) => {
    res.setHeader('Content-Type', 'application/json');
    const result = (req.session && req.session.googleLoginResult) || '';
    if (req.session && req.session.googleLoginResult) {
        // Reset for next time.
        req.session.googleLoginResult = '';
    }
    res.json({ account: result });
});

// Sign up with Google.
app.post('/google-student-signup-attempt', async (req, res, next) => {
    let payload;
    try {
        payload = await verifyGoogleCredential(req.body.credential);
    } catch (err) {
        console.error('Google credential verification failed:', err.message);
        return res.status(401).json({ account: 'invalid-google-token' });
    }
    try {
        // accountType is client-supplied: allowlist it. Privileged roles can
        // NOT be self-granted here (previously any role string was accepted).
        const accountType = ['student', 'instructor'].includes(
            req.query.accountType
        )
            ? req.query.accountType
            : 'student';
        const deviceType = req.query.deviceType;
        const referrerUsername = req.query.referrerUsername;

        const users = await query('SELECT * FROM users WHERE email = ?', [
            payload.email
        ]);
        let user = users[0];
        if (user) {
            await query(
                'UPDATE users SET is_deleted = 0, is_google_auth = 1 WHERE email = ?',
                [payload.email]
            );
            establishSession(req, user);
        } else {
            user = await createGoogleUser(payload, accountType, referrerUsername);
            establishSession(req, user);
        }
        req.session.googleLoginResult = 'new account';

        await identityBridge.ensureGoogleMapping(user.id, payload);

        if (user.role === 'student') {
            return res.redirect(
                deviceType === 'mobile' ? '/search' : '/skill-tree'
            );
        }
        return res.redirect('/students');
    } catch (err) {
        next(err);
    }
});

app.post('/google-editor-signup-attempt', async (req, res, next) => {
    let payload;
    try {
        payload = await verifyGoogleCredential(req.body.credential);
    } catch (err) {
        console.error('Google credential verification failed:', err.message);
        return res.status(401).json({ account: 'invalid-google-token' });
    }
    try {
        const users = await query('SELECT * FROM users WHERE email = ?', [
            payload.email
        ]);
        let user = users[0];
        if (user) {
            await query(
                'UPDATE users SET is_deleted = 0, is_google_auth = 1 WHERE email = ?',
                [payload.email]
            );
            establishSession(req, user);
        } else {
            user = await createGoogleUser(payload, 'editor', null);
            establishSession(req, user);
        }

        await identityBridge.ensureGoogleMapping(user.id, payload);

        return res.redirect('/');
    } catch (err) {
        next(err);
    }
});

// Login and out. -----------------------------------
// DELETE - redundant with below ("session-details")
// To show whether the user is logged in or not in the nav bar,
// and to pass there data to the Profile and Settings view.
app.get('/login-status', (req, res) => {
    res.json(Boolean(req.session && req.session.userName));
});

// Log in with username and password.
// For password decryption.
const bcrypt = require('bcrypt');

// bcrypt cost factor for locally stored password hashes.
const SALT_ROUNDS = 10;

app.post('/login-attempt', async (req, res, next) => {
    res.setHeader('Content-Type', 'application/json');
    try {
        const username = req.body.username;
        const password = req.body.password;

        // Look for user.
        const results = await query(
            `SELECT id, username, email, first_name, last_name, role, password, tenant_id
             FROM users
             WHERE users.username = ? AND users.is_deleted = 0`,
            [username]
        );

        // If username is found in DB.
        if (results.length > 0) {
            const user = results[0];
            // Check password.
            const matches = await new Promise((resolve) =>
                bcrypt.compare(
                    password || '',
                    user.password || '',
                    (err, ok) => resolve(!err && ok === true)
                )
            );
            // If password doesnt match.
            if (!matches) return res.json({ account: 'wrong-password' });

            // Populate user session data.
            req.session.isLoggedIn = true;
            req.session.userId = user.id;
            req.session.userName = username;
            req.session.firstName = user.first_name;
            req.session.lastName = user.last_name;
            req.session.role = user.role;

            // RFab bridge (section 3): an unmapped local password user gets
            // an RFab account provisioned with the same password (plaintext
            // is in hand right now). 409 EMAIL_EXISTS => stays unmapped;
            // attaching to an EXISTING RFab account needs a verified email
            // (Google login). Best-effort - never blocks the login.
            await identityBridge.ensurePasswordMapping(
                {
                    id: user.id,
                    email: user.email,
                    firstName: user.first_name,
                    lastName: user.last_name
                },
                password
            );

            // Send response to front end.
            return res.json({
                account: 'authorized',
                role: req.session.role,
                tenantId: user.tenant_id
            });
        }

        // No local user (section 3): an RFab user may be logging in with the
        // same email + password. Verify against RFab (username-as-email) and
        // auto-provision a local institute account on success.
        if (username && username.includes('@')) {
            const verified = await rfabBridge.verifyPassword({
                email: username,
                password
            });
            if (verified.ok) {
                const email = verified.email || username;
                // Never auto-provision over an existing local account that
                // owns this email (its address is unverified/untrusted).
                const emailRows = await query(
                    'SELECT id FROM users WHERE email = ?',
                    [email]
                );
                if (emailRows.length === 0) {
                    const user = await createLocalUserFromRfab(
                        verified,
                        email,
                        password
                    );
                    req.session.isLoggedIn = true;
                    req.session.userId = user.id;
                    req.session.userName = user.username;
                    req.session.firstName = user.first_name;
                    req.session.lastName = user.last_name;
                    req.session.role = user.role;

                    return res.json({
                        account: 'authorized',
                        role: user.role,
                        tenantId: null
                    });
                }
            }
        }

        return res.json({ account: 'no-account' });
    } catch (err) {
        next(err);
    }
});

/**
 * Auto-provision a local institute account for a verified RFab password user
 * (mirrors the Google signup path: uuidv7 id, role student, default avatar,
 * unlockInitialSkills) and map it as `password_verify`. The just-verified
 * password is stored (bcrypt) so subsequent logins also work locally.
 */
async function createLocalUserFromRfab(verified, email, password) {
    const nameParts = String(verified.name || '').trim().split(/\s+/);
    const data = {
        id: uuidv7(),
        username: email,
        email: email,
        first_name: nameParts[0] || null,
        last_name: nameParts.slice(1).join(' ') || null,
        password: await bcrypt.hash(password, SALT_ROUNDS),
        role: 'student'
    };
    await query('INSERT INTO users SET ?', data);
    await saveUserAvatarToAWS(data.id, DEFAULT_AVATAR);
    unlockInitialSkills(data.id).catch((err) =>
        console.error('Error unlocking initial skills:', err)
    );
    // Migration grant N/A: brand-new local user with 0 institute tokens.
    await identityBridge.mapPasswordVerifiedUser(data.id, verified.rfabUserId);
    return data;
}

app.post('/logout', (req, res) => {
    // Destroy the user session.
    req.session.destroy();
    res.end();
});

/*
 * User session
 */
// Send the session variables the frontend needs - an explicit allowlist,
// never the raw session object (it can contain transient auth state and
// store internals).
app.get('/get-session-details', (req, res) => {
    res.setHeader('Content-Type', 'application/json');
    const s = req.session || {};
    res.json({
        isLoggedIn: Boolean(s.userName),
        userId: s.userId || null,
        userName: s.userName || null,
        firstName: s.firstName || null,
        lastName: s.lastName || null,
        role: s.role || null
    });
});

/*
 * App settings
 */
// To get the app settings.
app.get('/api/settings', (req, res, next) => {
    if (req.session.userName) {
        let sqlQuery = `
        SELECT *
        FROM settings;`;
        conn.query(sqlQuery, (err, results) => {
            try {
                if (err) {
                    throw err;
                }
                res.json(results);
            } catch (err) {
                next(err);
            }
        });
    } else {
        res.redirect('/');
    }
});

// Edit app settings.
app.put(
    '/api/settings/edit',
    isAuthenticated,
    isPlatformAdmin,
    (req, res, next) => {
        if (req.session.userName) {
            let sqlQuery = `UPDATE settings SET ? WHERE id = 1`;
            const data = req.body;
            conn.query(sqlQuery, data, (err, results) => {
                try {
                    if (err) {
                        throw err;
                    }
                    res.end();
                } catch (err) {
                    next(err);
                }
            });
        }
    }
);

app.get('/sitemap.xml', (req, res) => {
    const rootUrl = 'https://parrhesia.io';
    const today = new Date().toISOString();
    const routes = [
        {
            path: '',
            priority: 1.0
        },
        {
            path: '/skill-tree',
            priority: 1
        },
        {
            path: '/login',
            priority: 0.6
        },
        {
            path: '/student-signup',
            priority: 0.6
        },
        {
            path: '/editor-signup',
            priority: 0.5
        },
        {
            path: '/skills',
            priority: 1
        }
    ];

    const sqlQuery =
        'SELECT * FROM skills WHERE skills.is_deleted = 0 AND ( type = "regular" OR type = "super" )';
    conn.query(sqlQuery, (err, results) => {
        if (err) {
            return res.status(500).send('Internal Server Error');
        }

        results.forEach((element) => {
            routes.push({
                path: `/skills/${element.name.replace(/ /g, '_')}`,
                priority: 1.0
            });
        });

        // Create the XML structure
        let sitemap = xmlbuilder
            .create('urlset', { encoding: 'UTF-8' })
            .att('xmlns', 'http://www.sitemaps.org/schemas/sitemap/0.9');

        // Add URLs to the sitemap
        routes.forEach((route) => {
            sitemap
                .ele('url')
                .ele('loc', `${rootUrl}${route.path}`)
                .up() // Ensured no extra slash added
                .ele('lastmod', today)
                .up()
                .ele('changefreq', 'weekly')
                .up()
                .ele('priority', route.priority)
                .up(); // Use route.priority as a number
        });

        // Send the XML sitemap as the response
        res.header('Content-Type', 'application/xml');
        res.send(sitemap.end({ pretty: true }));
    });
});

const environment = process.env.NODE_ENV;

app.get('/*', async (_req, res) => {
    const data = {
        environment,
        manifest: await parseManifest()
    };

    res.render('index.html.ejs', data);
});

const parseManifest = async () => {
    if (environment !== 'production') return {};

    const manifestPath = path.join(path.resolve(), 'dist', 'manifest.json');
    const manifestFile = await fs.readFile(manifestPath);

    return JSON.parse(manifestFile);
};

/**
 * API Response
 *
 * @return response()
 */
function apiResponse(results) {
    return JSON.stringify({ status: 200, error: null, response: results });
}

/*------------------------------------------
--------------------------------------------
Server listening
--------------------------------------------
--------------------------------------------*/
// app.listen(3000, () => {
//     console.log('Server started on port 3000...');
// });

server.listen(3000, () => {
    console.log('sever listening on *:3000');
});
