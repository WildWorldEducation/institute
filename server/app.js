...
const saltRounds = 10;
let ids = [
    24, 34, 41, 47, 52, 55, 56, 57, 66, 67, 69, 76, 77, 78, 79, 83, 84, 85, 86,
...
async function hashPasswords(index) {
    let sqlQuery1 = `
    SELECT password FROM users WHERE id = ${ids[index]};`;
    conn.query(sqlQuery1, (err, results) => {
        try {
            if (err) {
                throw err;
            ...
            console.log('id: ' + ids[index] + ', password: ' + password);
            if (password == undefined) {
                ...
                bcrypt.hash(
                    ...
                    function (err, hashedPassword) {
                        ...
                        let sqlQuery2 = `UPDATE users SET password = '${hashedPassword}'
                WHERE id = ${ids[index]};`;
