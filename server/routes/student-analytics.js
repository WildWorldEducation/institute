...
router.get(
    '/failed-assessments-by-subject/student/:studentId',
    async (req, res, next) => {
        // Check if logged in.
        if (req.session.userName) {
            res.setHeader('Content-Type', 'application/json');
            ...
            WHERE is_mastered = 1
            AND users.id = ${conn.escape(req.params.studentId)})  
...
router.get(
    '/passed-assessments-by-subject/student/:studentId',
    async (req, res, next) => {
        // Check if logged in.
        if (req.session.userName) {
            res.setHeader('Content-Type', 'application/json');
...
router.get(
    '/attempted-assessments-by-subject/student/:studentId',
    async (req, res, next) => {
        // Check if logged in.
        if (req.session.userName) {
            res.setHeader('Content-Type', 'application/json');
        ...
        if (req.session.userName) {
            ...
            conn.query(sqlQuery, (err, results) => {
                try {
                    ...
                    results = fillMissingDates(results);
...
/* Get number of assessments failed more than once, by root subject */
router.get(
    '/failed-assessments-by-subject/instructor/:instructorId',
    async (req, res, next) => {
        // Check if logged in.
        if (req.session.userName) {
            res.setHeader('Content-Type', 'application/json');
...
router.get('/total-skill-mastered/:studentId', (req, res, next) => {
    ...
    if (req.session.userName) {
        let sqlQuery = `
                        SELECT COUNT(*) AS total_mastered
                        FROM user_skills
                        JOIN skills
                        ON skills.id = user_skills.skill_id
                        WHERE is_mastered = 1
