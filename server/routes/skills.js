...
router.get('/url/:skillUrl', (req, res, next) => {
    ...
    res.setHeader('Content-Type', 'application/json');
    // Get skill.
    const sqlQuery = `SELECT s.id, s.name, s.url, s.parent, s.intro_sentence, s.type, s.level, s.image_url, s.image_thumbnail_url, s.icon_url, s.icon, s.mastery_requirements,
