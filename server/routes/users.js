                    ...
                    let query2 = conn.query(sqlQuery2, data, (err, results) => {
                        try {
                            if (err) {
                                ...
                                if (results.length > 0) {
                                    res.json({ account: 'email already taken' })
                                }
                                else {
                                    ...
                                    let query3 = conn.query(sqlQuery3, data, (err, results) => {
                                        try {
                                            if (err) {
                                                ...
                                                WHERE skill_tree.users.email = '` + req.body.email + `';`;
                                                let query = conn.query(sqlQuery4, (err, results) => {
                                                    try {
                                                        if (err) {
                                                            throw err;
                                                        }
...
router.get('/showId/:username', (req, res, next) => {
    if (req.session.userName) {
        res.setHeader('Content-Type', 'application/json');
        let sqlQuery = "SELECT id FROM users WHERE username = '" + req.params.username + "'";
        let query = conn.query(sqlQuery, (err, results) => {
            try {
                if (err) {
                    throw err;
    ...
    if (req.session.userName) {
        ...
        let query = conn.query(sqlQuery1, (err, results) => {
            try {
                ...
                let sqlQuery2 = "DELETE FROM user_skills WHERE user_id=" + req.params.id;
...
router.put('/:id/edit', (req, res, next) => {
    if (req.session.userName) {
        ...
        let sqlQuery = "UPDATE users SET first_name='" + req.body.firstname + "', last_name = '" + req.body.lastname + "', username = '" + req.body.username + "', email = '" + req.body.email + "', password = '" + req.body.password + "', avatar = '" + avatar + "', role = '" + req.body.role + "' WHERE id=" + req.params.id;
        let query = conn.query(sqlQuery, (err, results) => {
            try {
                if (err) {
                    throw err;
...
router.put('/:id/skilltree-theme', (req, res, next) => {
    if (req.session.userName) {
        let sqlQuery = "UPDATE users SET skilltree_theme='" + req.body.theme + "' WHERE id=" + req.params.id;
        let query = conn.query(sqlQuery, (err, results) => {
            try {
                if (err) {
                    throw err;
