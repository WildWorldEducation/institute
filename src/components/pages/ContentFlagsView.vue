...
export default {
    ...
    methods: {
            ...
            fetch('/content-flags/list')
                ...
                .then(() => {
                    for (let i = 0; i < this.contentFlags.length; i++) {
                         ...
                         */
                        const flagRow = {};
                        flagRow.isRewardGiven = flag.is_reward_given;
        ...
        // Reward user with a reputation point.
        handleRewardUser(contentId, userId, flagId) {
            if (
                !confirm(
                    'Are you sure you want to give this user a repuatation point?'
                )
            ) {
                return;
            } else {
                // increase user's reputation points
                let contentType = 'content_flag';
                ...
                fetch(url, requestOptions).then(() => {
                    // hide button from now on
                    var url = '/content-flags/' + flagId + '/mark-reward-given';
                    const requestOptions = {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' }
                    };
                    fetch(url, requestOptions)
                        ...
                        .then(() => {
                            alert(
                                'User has been rewarded with one reputation point for that flag.'
                            );
                        });
                });
            }
            ...
            >
                ...
                >
                    ...
                    >
                        <!-- Reward button -->
                        <button
                            v-if="isRewardGiven == 0"
                            b-tooltip.hover
                            title="Reward user with a reputation point"
                            class="btn btn-primary"
                            @click="
                                handleRewardUser(contentId, user.id, flagId)
                            "
                        >
                            <!-- Award icon -->
                            <svg
