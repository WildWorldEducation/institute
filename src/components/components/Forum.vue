...
export default {
        ...
        for (let k = 0; k < this.users.length; k++) {
          if (this.posts[j].user_id == this.users[k].id) {
            this.posts[j].studentName =
              this.users[k].first_name + ' ' + this.users[k].last_name;
  ...
  methods: {
    ...
    voteUp(resourceIndex, resourceId, hasVoted) {
      if (hasVoted) {
        fetch(
          '/user-votes/' + this.user.userId + '/' + resourceId + '/edit/cancel',
          {
            method: 'PUT',
            headers: {
        ...
        fetch(
          '/user-votes/' + this.user.userId + '/' + resourceId + '/edit/up',
          {
            method: 'PUT',
            headers: {
              'Content-Type': 'content/type',
    ...
    voteDown(resourceIndex, resourceId, hasVoted) {
      if (hasVoted) {
        ...
        fetch(
          '/user-votes/' + this.user.userId + '/' + resourceId + '/edit/down',
          {
            method: 'PUT',
            headers: {
              'Content-Type': 'content/type',
...
<template>
  <div class="container-fluid">
    <div id="first-forum-row" class="row justify-content-md-between mt-3">
      ...
      <div id="add-resource-column" class="col-12 col-md-6 mt-3 mt-md-0">
        <div class="d-flex flex-row-reverse center-header">
          <router-link :to="'/resources/add/' + skillId" class="btn green-btn" role="button">Add resource&nbsp;&nbsp;
            <!-- Plus sign -->
      ...
      <div class="row forum-container mt-3" v-for="post in orderedAndNamedPosts">
        <div class="row">
          <div class="d-flex flex-row justify-content-end">
            ...
            <router-link v-if="post.user_id == user.userId" :to="'/resources/edit/' + post.id" class="btn" role="button">
              <!-- Pencil icon -->
        ...
        <div class="row">
          <div class="col post-user-row">
            <div id="user-avatar">
              <img :src="post.userAvatar" class="user-avatar-img" alt="user avatar" />
            </div>
      ...
      <div id="myModal" class="modal">
        ...
        <div class="modal-content">
          ...
          <div style="display: flex; gap: 10px">
            <button type="button" class="btn btn-danger" @click="deletePost(this.resourceId)">
              Yes
            </button>
