...
export default {
            ...
            for (let j = 0; j < this.posts.length; j++) {
                for (let k = 0; k < this.users.length; k++) {
                    if (this.posts[j].user_id == this.users[k].id) {
                        this.posts[j].studentName = this.users[k].first_name + " " + this.users[k].last_name;
                    }
                }
            }

            // Ordering by vote.
            var sortedPosts = this.posts.sort(({ voteCount: a }, { voteCount: b }) => b - a);
    ...
    methods: {
        ...
        voteUp(resourceIndex, resourceId, hasVoted) {
            if (hasVoted) {
                fetch("/user-votes/" + this.user.userId + "/" + resourceId + "/edit/cancel", {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'content/type'
                    },
                    body: {}
                }).then(response => this.getPostVote(resourceIndex, resourceId));
            }
            else {
                fetch("/user-votes/" + this.user.userId + "/" + resourceId + "/edit/up", {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'content/type'
                    },
        ...
        voteDown(resourceIndex, resourceId, hasVoted) {
            if (hasVoted) {
                ...
                fetch("/user-votes/" + this.user.userId + "/" + resourceId + "/edit/down", {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'content/type'
                    },
...
<template>
    <h2>Resources</h2>
    <div class="row mt-3">
        <div class="d-flex flex-row-reverse center-header">
            <router-link :to="'/resources/add/' + skillId" class="btn green-btn" role="button">Add
                resource&nbsp;
                <!-- Plus sign -->
    ...
    <div class="row forum-container mt-3" v-for="post in orderedAndNamedPosts">
        <div class="row">
            <div class="d-flex flex-row justify-content-end">
                <button @click="voteUp(post.index, post.id, post.userUpVote)" class="btn" type="button">
                ...
                <button @click="voteDown(post.index, post.id, post.userDownVote)" class="btn" type="button">
                    <svg width="34" height="27" viewBox="0 0 34 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                ...
                <router-link v-if="post.user_id == user.userId" :to="'/resources/edit/' + post.id" class="btn"
                    role="button">
                    <!-- Pencil icon -->
                ...
                <button v-if="post.user_id == user.userId || user.role == 'admin'" type="button" class="btn"
        ...
        <div id="myModal" class="modal">
            ...
            <div class="modal-content">
                ...
                <div style="display:flex">
                    <button type="button" class="btn btn-danger" @click="deletePost(resourceId)">Yes</button>
                    <button type="button" class="btn btn-dark" @click="closeWarningModal()">No</button>
                </div>
            </div>
