  ...
  methods: {
    getAlgorithm() {
      ...
      for (let i = 0; i < flattenedSkillChildren.length; i++) {
        for (let j = 0; j < sortedRootDescendants.length; j++) {
          if (
            flattenedSkillChildren[i].id == sortedRootDescendants[j].data.id
          ) {
    ...
    drawChart(root) {
      ...
      for (let i = 0; i < root.links().length; i++) {
        ...
        // Source node.
        var sourceX =
          Math.cos(root.links()[i].source.x) * root.links()[i].source.y;
        var sourceY =
          Math.sin(root.links()[i].source.x) * root.links()[i].source.y;
        link.moveTo(sourceX, sourceY);

        // Target node.
        var targetX =
          Math.cos(root.links()[i].target.x) * root.links()[i].target.y;
        var targetY =
          Math.sin(root.links()[i].target.x) * root.links()[i].target.y;
