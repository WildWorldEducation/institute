...
export default {
    name: 'CohortPassedAssessmentsHorizontalBarChart',
    props: ['data', 'colour'],
    data() {
        return {
            padding: 60
    ...
    mounted() {
        const container = d3.select(
            '#cohort-passed-assessments-chart-container'
        );
        ...
        const svg = d3
            .select('#cohort-passed-assessments-chart-container')
            .append('svg')
            .attr('width', width)
            .attr('height', height)
...
<template>
    <div id="cohort-passed-assessments-chart-container"></div>
</template>
