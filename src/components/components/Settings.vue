...
export default {
    setup() {
        ...
        return {
            userDetailsStore, settingsStore
        }
    },
    data() {
        return {
...
<template>
    <div class="container">
        ...
        <div v-if="userDetailsStore.role == 'admin'">
            ...
            <p>time for student skills to degrade (in days): {{ settingsStore.settings.skill_degradation_days }}</p>
            <p><router-link to="/settings/edit" class="btn green-btn" role="button">
                    Edit&nbsp;
                    <!-- Pencil icon -->
