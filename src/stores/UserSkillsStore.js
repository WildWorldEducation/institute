        ...
        async MakeMastered(userId, skill) {
            ...
            fetch(url, requestOptions).then(() => {
                ...
                if (skill.type != 'sub') {
                    ...
                    for (let i = 0; i < childSkills.length; i++) {
                        ...
                        // If super type skills, make their subskills accessible.
                            ...
                            for (
                                let j = 0;
                                j < skillsStore.skillsList.length;
                                j++
                            ) {
                                if (
                                    skillsStore.skillsList[j].parent ==
                                        childSkills[i].id &&
                                    skillsStore.skillsList[j].type == 'sub'
                                ) {
                                    subSkills.push(
                                        skillsStore.skillsList[j].id
                                    );
                                }
                            }
                        }
                ...
                // If this skill is a sub skill.
                    ...
                    for (let i = 0; i < skillsStore.skillsList.length; i++) {
                        if (
                            skillsStore.skillsList[i].parent == skill.parent &&
                    ...
                    for (let i = 0; i < this.unnestedList.length; i++) {
                        for (let j = 0; j < siblingSkills.length; j++) {
                            if (
                                siblingSkills[j].id == this.unnestedList[i].id
                            ) {
